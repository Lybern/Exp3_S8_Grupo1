...
public class UsuarioController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Lista de usuarios obtenida exitosamente", content = @Content(mediaType = "application/json"))
        })
        public ResponseEntity<PagedModel<EntityModel<UsuarioResponseDTO>>> listarUsuarios(
                ...
                List<EntityModel<UsuarioResponseDTO>> usuariosModel = usuariosPage.getContent()
                                .stream()
        ...
        public ResponseEntity<EntityModel<UsuarioResponseDTO>> obtenerUsuarioPorId(
                ...
                // 1. Intentar obtener el usuario. Si no existe, lanzamos la excepción que el
                // handler traduce a 404 estructurado.
                Usuario u = usuarioService.findById(id)
                                .orElseThrow(() -> new NotFoundException(
                                                "El usuario con el ID " + id + " no existe en el sistema."));

                // 2. Construir el objeto de respuesta con los enlaces HATEOAS correspondientes
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Usuario creado exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UsuarioResponseDTO.class), examples = @ExampleObject(value = """
                                        {
                                          "id": 1,
                                          "username": "admin",
                                          "roles": ["ADMINISTRADOR"],
        ...
        public ResponseEntity<EntityModel<UsuarioResponseDTO>> actualizarUsuario(
                ...
                Usuario usuarioExistente = usuarioService.findById(id)
                                .orElseThrow(() -> new NotFoundException("El usuario con el ID " + id
                                                + " no existe para poder ser actualizado."));
